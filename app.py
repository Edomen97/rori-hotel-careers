...
if CLOUDINARY_AVAILABLE:
    ...
    try:
        ...
        if app.config.get("CLOUDINARY_URL"):
            ...
            print(
                "[CLOUDINARY] Configured using CLOUDINARY_URL"
            )
...
def is_cloudinary_cv(filename):
    ...
    return bool(
        filename and
        filename.startswith("cloudinary:")
    )
...
def parse_cloudinary_cv(filename):
    ...
    if (
        ...
        or
        extension
        not in app.config["ALLOWED_EXTENSIONS"]
    ):
        return None, None
...
def save_uploaded_file(file):
    ...
    if not file or not file.filename:
        print("[CV] No file received.")
        return None

    if not allowed_file(file.filename):
        print(
            "[CV] Invalid extension:",
            file.filename
        )
        return None
    ...
    try:
        ...
        if not public_id:
            ...
            print(
                "[CV] Cloudinary returned no public_id."
            )

            return None

        stored_reference = (
            f"cloudinary:{public_id}|{extension}"
        )
...
def apply(job_id):
    ...
    if form.validate_on_submit():
        ...
        application = Application(
            ...
            skills=
            form.skills.data.strip(),

            languages=
            form.languages.data.strip(),
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        default_username = app.config.get(
            "HR_USERNAME",
            "admin"
        )

        default_hash = app.config.get(
            "HR_PASSWORD_HASH"
        )
...
def admin_change_password():
    ...
    if form.validate_on_submit():
        ...
        if (
            ...
            flash(
                "ያስገቡት Current Password ትክክል አይደለም!",
                "danger"
            )
...
@app.route(
    "/admin/application/<int:app_id>/cv/download"
)
@admin_required
...
def admin_job_toggle(
    ...
    try:
        ...
        log_audit(
            "Changed Vacancy Status",
            (
                f"{job.title} changed to "
                f"{job.status}"
            ),
            "Job",
            job.id
...
def migrate_existing_database():
    ...
    try:
        ...
        print(
            "[DB WARNING] Job status backfill:",
            e
        )
    ...
    try:
        ...
        print(
            "[DB WARNING] published_at backfill:",
            e
        )
